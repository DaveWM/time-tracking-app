...
* [`src/cljs/time_management_client/`](src/cljs/time_management_client/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/time_management_client/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/time_management_client/`](test/cljs/time_management_client/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `time-management-client` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/time_management_client/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns time-management-client.example
  (:require [time-management-client.config :as config])
